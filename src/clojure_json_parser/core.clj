  ...
  (-->
    ...
    (cond
      ...
      (str/starts-with? input "\\") (-->
                                      (let [esc-char-matcher (->>
                                                               (subs input 1)
                                                               (parse-esc-char)
                                                               (or (parse-unicode-char (subs input 1))))])
...
(defn parse-object-key-value [input]
  (-->
    ...
    (let [object-key (get object-key-matcher 0)
          input (get object-key-matcher 1)])
...
(defn parse-object [input]
  (cond
    ...
    :else (-->
            ...
            (when (and object-key-values
                    (str/starts-with? (str/triml (get object-key-values 1)) "}")))
...
(defn parse-json [input]
  (let [value-match (parse-value input)]
    (when (and value-match
            (= (str/triml (get value-match 1)) ""))
      (get value-match 0))))

(defn test-json-parser []
  (let [test-files
        (->>
          (io/file "./test/clojure_json_parser/cases")
          (file-seq)
          (filter #(.isFile %))
          (map #(.getAbsolutePath %)))]
    (map #(list % (->>
